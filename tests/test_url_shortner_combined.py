import json
from urlshortner.url_shortner_combined import create_url, get_url, check_valid_url, update_blacklist


def test_create_url() -> None:
    assert json.loads(create_url(
        {"headers": {"url": "https://www.shreyasgaonkar.com/"}})["body"]) == "https://trim.live/8f0fe3"


def test_get_url() -> None:
    assert json.loads(get_url({"path": "/8f0fe3"})
